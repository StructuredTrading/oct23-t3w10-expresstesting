...
describe("v2/ functionality", () => {
	...
	it("v2/headercheck GET receives an auth header correctly", async () => {
		const response = await request(app).get("/v2/headercheck").auth("sam was here", {type: "bearer"});

        expect(response.body.data).toBe("Bearer some incorrect text to force a fail");
	});
